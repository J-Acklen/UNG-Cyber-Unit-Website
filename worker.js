...
// ─── Server-rendered topic lesson content ─────────────────────────────────────
// Mirrors the client-side renderers in public/js/main.js (renderContent and its
// helpers, plus getTopicSVG) so a /topic/:id response has real lesson content in
// the initial HTML instead of a "Loading topic..." placeholder. Without this,
// crawlers that don't (fully) execute JS see every topic page as identical thin
// content — which is exactly what got /topic/02 flagged as a Soft 404 in Search
// Console. main.js re-renders the same markup client-side once it fetches
// /api/topic/:id (for per-user progress etc.); this SSR copy just has to produce
// equivalent output. Keep these two in sync by hand if either changes.
...
// Per-topic inline SVG illustration — mirrors public/js/main.js's getTopicSVG().
